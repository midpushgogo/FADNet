...
def build_comb(left_feature, right_feature, left_compressed, right_compressed, max_disp=40, zero_volume=None, comb=None):
    b, c, h, w = left_feature.shape
    cost_volume_correlation = left_feature.new_zeros(b, max_disp, h, w)
    cost_volume_con = left_feature.new_zeros(b, 2 * left_compressed.shape[1], max_disp, h, w)
    for i in range(max_disp):
        if i > 0:
            cost_volume_correlation[:, i, :, i:] = (left_feature[:, :, :, i:] *
                                                    right_feature[:, :, :, :-i]).mean(dim=1)
            cost_volume_con[:, :, i, :, i:] = torch.cat((left_compressed[:, :, :, i:], right_compressed[:, :, :, :-i]),
                                                        dim=1)
        else:
            cost_volume_correlation[:, i, :, :] = (left_feature * right_feature).mean(dim=1)
            cost_volume_con[:, :, i, :, :] = torch.cat((left_compressed, right_compressed), dim=1)
